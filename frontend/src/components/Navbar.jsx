    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-black border-b border-red-600">
      <div className="max-w-7xl mx-auto px-8 py-5 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <span className="font-mono text-sm text-green-400 tracking-widest">
            AVAILABLE
          </span>
        </div>
        ...
        {/* Mobile hamburger */}
        <button
          className="md:hidden font-mono text-sm text-gray-400 hover:text-white"
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-black border-t border-red-600 px-8 py-5 flex flex-col gap-5">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
